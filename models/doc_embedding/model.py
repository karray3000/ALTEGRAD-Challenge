...
class DocEmbeddingModel(BaseEstimator):
    ...
    def fit(self, X_train, y_train):
        ...
        #         shortest_paths = [len(nx.shortest_path(self.G, curr_host, target_host)) for target_host, target_label in self.train_hosts_with_labels.items()
        #                           if target_label == labels[j] and target_host != curr_host and nx.has_path(self.G, curr_host, target_host)]
        #         if len(shortest_paths) == 0: shortest_paths = [100000]
        #         X_train_graph[i, j] = np.min(shortest_paths)
        ...
        res = self.full.fit(X_train_text, y_train)
    ...
    def predict_proba(self, X_test):
        ...
        #                           if target_label == labels[j] and target_host != curr_host and nx.has_path(self.G, curr_host, target_host)]
        #         if len(shortest_paths) == 0: shortest_paths = [100000]
        #         X_test_graph[i, j] = np.min(shortest_paths)
        ...
        return self.full.predict_proba(X_test_text)
